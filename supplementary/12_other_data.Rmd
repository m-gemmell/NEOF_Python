...
Often scientists store data in delimited text files which are typically separated by commas or tabs.
We can read these into Python, loop through them and manipulate their values using similar, or identical,
approaches to those we used with our sequence data previously.

We are going to load a further example dataset; a typical table of comma delimited data, and use the 
Python module 'pandas' to filter and manipulate the data.
...
The data we will be looking at contains the following variables: SampleID, age, gender, nationality, diverity of gut microbiome, and a BMI category.

## Reading .csv files (or similar) with pandas

<center>![](../images/pandas_icon.jpg){height=300px}</center>


The Python module [pandas](https://pandas.pydata.org/) specifically for reading delimited text files. 
It pre-processes text data files and makes them easier to process and use in analysis.

Just as we did earlier, first we establish a link to the file we are to import with using "open" and "with".

Modify the commands from the previous chapter to the following.

```
with open(wd + "data/patients_modified.csv", 'w') as output_file:
    with open(wd + "data/patients_clean.csv", 'r') as input_file:
        for row in input_file:
            print(row)
            #output_file.write("Study_2022," + row)
```

We can see the comma separated data typical of a .csv file printed in the terminal, 
however we can use specific Python data processing tools to make the data more usable.

Pandas allows us to access
certain elements within each row? For example, the gender of each patient.

Start a new code block, load pandas and use the pandas method read_csv to input the file:
```
import pandas

patients = pandas.read_csv(wd + "data/patient_data.csv")
```

Now we have read our data into the object 'patients', we can take a look at how pandas has stored the data.

```
print(print(patients.dtypes))
```
...
We can access a single column of data by it's variable name assigned by pandas.
...
To access individual records, we need to first tell pandas which column contains the index, or sample names in this case.
In the case of the patients.csv data file, the identifiers are in the first column, which Python counts as column zero.

Modify the command above to the following to add the index information, and print the data associated with sample #21.

```
patients = pandas.read_csv(wd + "data/patient_data.csv", index_col=0)
...
<i>*NOTE* the double square brackets containing the variable names: [[ ]]
(this ie because we are choosing columns by index - which is done by passing an argument in square brackets,
*AND* we're passing a *list* of the columns to select which is *also* defined by being within square brackets).</i>
...
this new dataframe is also in the pandas format.
...
YOu can always fefer back to chapter *???* for a refresher on how selection statements and operators work
...
Pandas provides useful methods for working with data tables, as we have seen. All of the previous approaches
