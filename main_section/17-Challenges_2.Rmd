...
__Tip__: The __method__ `.values()` can extract the __values__ from a __dictionary__. These __strings__ can be looped through, e.g. `for value in national_animals.values()` try the below code to __print__ out the values.

```{python, eval=FALSE}
#Loop through and print values
for value in national_animals.values():
  print(value)
```
