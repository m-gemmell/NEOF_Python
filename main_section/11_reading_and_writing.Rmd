...
To read/write a file in Python we open a data stream using a statement like:
...
We need to specify the mode by which the file is opened. The most common modes are:
...
The 'with' statement, which is wrapped around the open() statement, gives this data stream a variable name (input_file) and also
performs important 'under-the-hood' cleaning up that stops files becoming corrupted.

## Read a data file
<center>![](images/read.webp){height=220px}</center>
Open a new script in Gedit. Add the following to open a data stream to <b>read</b> the provided file 'patients_data.csv',
and then add a 'for' loop to print each line of the file.
...
head data/patients_modified.csv
```
...
Return to the script in Gedit that was concerned with the FastQ data (named script02.py).
...
```
with open(wd + "data/R_undulata_filtered_seqs.fastq", 'w') as output_file:
